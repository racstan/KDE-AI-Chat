...
└── contents/
    ...
    ├── ui/
    │   ├── main.qml                # ~5,870 lines — Core chat widget
    │   ├── ConfigGeneral.qml       # ~4,442 lines — Settings panel
    │   ├── ScheduleDialog.qml      # ~1,293 lines — Schedule editor
...
- **Provider Selection**: Dropdown of 17+ AI providers with per-provider URL, API key, and model fields.
...
### 3. `translations.js` — Translation Engine
...
### 4. `doc_extractor.py` — File Attachment Extractor
...
### 5. `kde-ai-scheduler.py` — Scheduling Daemon
...
- Maintains a run history (up to 100 entries).
- Responds to SIGHUP to reload schedules without restart.

### 6. `config.qml` — Settings Tab Registration
...
SSE events via /v1/event → handleOpenCodeEvent()
  ├─ message.updated → Streaming token updates
...
- **Provider settings**: URL, API key, model for each of 17+ providers
