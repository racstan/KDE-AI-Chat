...
QQC2.ScrollView {
    ...
    readonly property string providerGuideText: {
        ...
        if (provider === "openai")
            ...
            return translate("<b>Pollinations.ai Image Generation Guide:</b><br/>" + "• <b>Free &amp; No API key needed!</b> Pollinations.ai generates images from text prompts at no cost.<br/>" + "• Select <b>[Image] Pollinations.ai</b> as your provider.<br/>" + "• Type a description of the image you want (e.g. <i>\"a sunset over mountains in watercolor style\"</i>).<br/>" + "• The AI will generate and display the image in the chat.<br/>" + "• Default model: <code>flux</code>. Other options: <code>flux-realism</code>, <code>turbo</code>.");
        else if (provider === "huggingface-image")
            return translate("<b>HuggingFace Image Generation Guide:</b><br/>" + "1. (Optional) Get a free API key at <b>huggingface.co → Settings → Access Tokens</b> for higher rate limits.<br/>" + "2. Paste it into the <b>HuggingFace Image key</b> field below.<br/>" + "3. Choose a model (e.g. <code>stabilityai/stable-diffusion-xl-base-1.0</code>).<br/>" + "4. Type a description and the AI will generate an image.<br/>" + "5. Free tier: ~300 requests/day without API key.");
        else if (provider === "together-image")
